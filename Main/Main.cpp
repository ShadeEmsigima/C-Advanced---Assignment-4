...
//voeg toe aan vector, kijk of jij groter bent dan de laatste, mag jij voor de laatste gaan staan
	...
	void sortVectorDouble(std::vector<double> a) { //for a vector that you get
		...
		//std::cout << "Sorting vector of type double" << std::endl; //print what youll do
		std::cout << "First 2 values are "<< tempVec.at(0)<< " " << tempVec.at(1) << std::endl;//check which variables tempvec has

		//now sort the tempVec, larger to smaller value:
		if (tempVec.at(0) < tempVec.at(1)) { //if the values are not in decending order
			double tempVar = tempVec.at(0);//save the first to temporary variable
			tempVec.erase(tempVec.begin());//remove the first
